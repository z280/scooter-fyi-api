...
| Ride Mode A4 — leaderboard endpoint + privacy semantics | Implemented. `src/area_leaders.py:recompute()` (cron `15 9 * * * python -m src.cli recompute_area_leaders`) full-replaces `sql/048_h3_r8_area_leaders.sql`'s `h3_r8_area_report`/`h3_r8_area_leaders`/`h3_r8_area_leader_runs` from a trailing-28-day, `status='confirmed'` window over a universe unioned from `device_history`/`device_state`/`user_points`, tie-break `points DESC, first_point_at ASC, account_id ASC`, top 3 stored per cell. `src/api_leaderboard.py`: `GET /api/v1/leaderboard/map` (public) live-joins that report against `accounts`, applying `show_in_leaderboards`/`show_public_username`/NULL-`display_name` filtering at READ time — never baked into the nightly recompute — so an opt-out takes effect on the very next request; a skipped rank falls through (leader = highest surviving stored rank, runners_up = the rest, <=3 total; total_points/distinct_earners stay unfiltered, since they carry no identity). Colors are live-joined too; an unclaimed `ruling_color`/`ruling_border_color` pair also nulls `ruling_alpha` in the payload rather than leaking its `NOT NULL DEFAULT 0.60` schema value. ETag is deliberately **not** run-keyed (§11.4's literal shape would 304 an opted-out rider back into view until the next daily run): `W/"arealb:<computed_at epoch>:<sha256(canonical cells json)[:16]>"`, so either component changing busts the tag. `GET /api/v1/private/area-leaders` (`src/api_private.py`) is the unfiltered admin sibling — every stored rank, real account ids, no privacy filtering. Three-address rule: `src/api_meta.py:_PRIVACY` gained `h3_r8_area_leaders` (derived nightly from the confirmed points ledger, fully replaced each run, deleted with the account via `ON DELETE CASCADE`, publicly exposed only through the read-time visibility filters above); `src/templates/legal/privacy_policy.html` carries the matching row. |
