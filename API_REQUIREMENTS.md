...
| Along the Way — ride specs (Phase 1) | Implemented — `sql/080_ride_specs.sql` adds `user_preferences` kind `ride_spec` (name required, one per `(account, name)`, `idx_user_prefs_spec_name`); `src/api_preferences.py` serves `GET/PUT/DELETE /api/v1/profile/ride-specs[/{name}]`, 5 per account (`MAX_RIDE_SPECS`), 16 KB per blob. A spec is a rider's "ideal scooter" — models, required features, min battery, min quality, plus the `must` list that marks which of those are HARD. Blob is **opaque here**: the strict shape check belongs to the trip search (`POST /api/v1/trip/candidates`, ALONG_THE_WAY_PLAN.md §6), which is the endpoint that disqualifies vehicles against it. Both guarded CHECK rewrites follow sql/050's shape, so a replay after a fifth kind lands is a no-op. |
